...
def main():
	...
	description: str = (
						"Transform exported whatsapp discussions "
						"into ready-for-import slack.com threads as a csv file.")
	delimiter: str = ","
	...
	parser.add_argument(
		"input", type=argparse.FileType('r'), help="Input filename")
	parser.add_argument(
		"-c", 
		"--channel", 
		default=channel_name, 
		help="Slack.com channel name, default: "+channel_name)
	parser.add_argument(
		"-d", "--delimiter", 
		default=delimiter, 
		help="CVS delimiter, default: '" + delimiter + "'")
	parser.add_argument(
		"-o", "--output", type=argparse.FileType('w'), help="Output filename")
	parser.add_argument(
		"-u", "--username", 
		action="store_true", 
		help="Modify usernames, default: 'FALSE'")
	# parser.print_help()
	...
	if (args.output is None):
		output_file: TextIO = open("Slack Import "+args.input.name, 'w')
	else:
		...
		for line in input_lines:
			...
			try:
				...
				if output_elements.get("content", None) is not None:
					new_line: str = FORMAT_STRING.format(
														delimiter, 
														int(output_elements["date"].timestamp()), 
														channel_name, output_elements["username"], 
														output_elements["content"])
					print(new_line)
				...
				# Oh, by the way, look for a username. 
				# The presence of a username followed by a colon 
				# is the only flag we can use.
				if line[23:].count(':') > 0: 
