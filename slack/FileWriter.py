...
class FileWriter(object):
	...
	def __init__(self, fileHandle:Union[TextIOWrapper,str], channel:str=None, delimiter:str=None, overrideUsername:bool=None):

		try:
			if isinstance(fileHandle, TextIOWrapper):
				if (fileHandle.mode.count("w")) == 0:
					raise FileNotFoundError("File needs to be in write mode")
	...
	def write(self, line:Dict[str,Union[str,datetime]]):
#		new_line:str = FORMAT_STRING.format(self._delimiter, 
#						int(line["date"].timestamp()), 
#						self._channel, 
#						line["username"], 
#						line["content"])
#		print(new_line)
